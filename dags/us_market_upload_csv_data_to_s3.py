...
class UploadCsvToS3:
    def __init__(self):
        ...
        # self.configuration_directory_name = "config" # local test
        ...
        # self.csv_directory_name = "data" # local test
        ...
        self.us_bucket_name = 'your bucket name'  # us market S3 bucket
    ...
    def upload_csv_to_s3(self):
        ...
        for file_path in self.csv_file_path_list:
            ...
            if market_region == "us":
                bucket_name = self.us_bucket_name
            else:
                continue

            print(f"bucket_name : {bucket_name}")
            df = pd.read_csv(file_path)
            target_date_list = df['xymd'].unique().tolist()

            print(f"target_date_list : {target_date_list}")

            for target_date in target_date_list:
                year, month, day = target_date.split('-')
                target_df = df[df['xymd'] == target_date]  # unique 값이 되어야 함
...
with DAG(
        dag_id="us_market_upload_stock_data_to_s3",
        start_date=datetime(2023, 9, 25, 9),
        schedule_interval='45 9 * * 2-6',  # UTC -> 한국 기준 화 - 토
        catchup=False,
        tags=["US", "UPLOAD", "CSV", "S3"]
) as dag:
